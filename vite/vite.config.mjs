...
function rewriteMovedCssAssetUrls(absoluteCssPath, originalCssDir, normalizedCssDir, legacySourceDir) {
    ...
    const rewrittenCssContent = cssContent.replace(
        ...
        (_, quote, assetPath) => {
            ...
            const rebasedAssetPath = getRebasedMovedCssAssetPath(
                cleanAssetPath,
                originalCssDir,
                normalizedCssDir,
                legacySourceDir
...
function getRebasedMovedCssAssetPath(assetPath, originalCssDir, normalizedCssDir, legacySourceDir) {
    const originalAssetPath = path.posix.normalize(path.posix.join(originalCssDir, assetPath));

    if (assetExistsInBuild(originalAssetPath)) {
        return path.posix.relative(normalizedCssDir, originalAssetPath);
    }
    ...
    const legacySourcePath = path.posix.normalize(path.posix.join(legacySourceDir, assetPath));
    const legacyAssetPath = mapSourceAssetToBuildAsset(legacySourcePath);

    if (!legacyAssetPath || !assetExistsInBuild(legacyAssetPath)) {
        return '';
    }

    return path.posix.relative(normalizedCssDir, legacyAssetPath);
}

function mapSourceAssetToBuildAsset(sourcePath) {
    return sourcePath.startsWith('resources/')
        ? sourcePath.replace(/^resources\//, '')
        : '';
}

function assetExistsInBuild(assetPath) {
    if (assetPath.startsWith('../') || path.posix.isAbsolute(assetPath)) {
        return false;
    }

    return fs.existsSync(path.resolve(pluginRoot, 'assets', assetPath));
}
