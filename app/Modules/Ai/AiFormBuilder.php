...
- Field type mapping:  
  - Fields related to payment → **element: 'multi_payment_component'**  
  - Fields related to donation → **element: 'custom_payment_component'**  
  - Fields related to payment quantity → **element: 'item_quantity_component'**  
...
EOD;
        ...
        $result = (new GeminiApi())->makeRequest($queryArgs);
        if (is_wp_error($result)) {
            throw new Exception($result->get_error_message());
        }

        $response = trim(Arr::get($result, 'candidates.0.content.parts.0.text', ''), '"');
