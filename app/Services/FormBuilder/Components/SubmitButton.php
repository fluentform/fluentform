    ...
    public function compile($data, $form)
    {
        ...
        if (isset($data['settings']['button_ui'])) {
            if ('default' == $data['settings']['button_ui']['type']) {
                $buttonText = $this->parseEditorSmartCode($data['settings']['button_ui']['text'], $form);
                $ariaLabel = wp_strip_all_tags($buttonText);
                $html .= '<button ' . $atts . ' aria-label="' . esc_attr($ariaLabel) . '">' . fluentform_sanitize_html($buttonText) . '</button>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $atts is escaped before being passed in.
            } else {
            ...
            $buttonText = $this->parseEditorSmartCode($data['settings']['btn_text'], $form);
            $ariaLabel = wp_strip_all_tags($buttonText);
            $html .= '<button ' . $atts . ' aria-label="' . esc_attr($ariaLabel) . '">' . fluentform_sanitize_html($buttonText) . '</button>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $atts is escaped before being passed in.
        }
