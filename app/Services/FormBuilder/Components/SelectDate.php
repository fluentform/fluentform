...
class SelectDate extends Select
{
    ...
    {
        $dateFormat = 'l'; // Example date format, replace with your actual format
        ...
        for ($i = 1; $i <= $daysInMonth; $i++) {
            $dateValue = date($dateFormat, strtotime("2023-01-$i"));
        
            // Check if the date has already been added, if not, add it to the options
            if (!in_array($dateValue, $addedDates)) {
                $options[] = [
                    'label' => $dateValue,
                    'value' => $dateValue,
                ];
            
                // Add the date to the list of added dates
                $addedDates[] = $dateValue;
            }
        }
