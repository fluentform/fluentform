...
class ActivationHandler
{
    ...
    private function maybeMigrateDB()
    {
        ...
        if (!get_option('fluentform_entry_details_migrated')) {
            SubmissionDetailsMigrator::migrate();
        }
    ...
    private function migrateGlobalAddOns()
    {
        ...
        $moduleStatuses = [];
        foreach ($possibleGlobalModules as $moduleName => $settingsKey) {
        ...
        foreach ($possibleMetaModules as $moduleName => $metaName) {
            $row = FormMeta::where('meta_key', $metaName)->count();
    ...
    private function maybeMigrateDefaultForms()
    {
        $hasForm = Form::get()->count();
        ...
        if (!$hasForm) {
            ...
            foreach ($forms as $index => $formJson) {
                ...
                $formId = Form::create($insertData)->id;
