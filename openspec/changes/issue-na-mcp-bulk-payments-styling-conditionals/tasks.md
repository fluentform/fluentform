...
## 9. Default payment provider (free core — revised; the Pro-companion plan was superseded, fluentformpro#223 closed)

> The payment module (`app/Modules/Payments`: OrderData, PaymentHelper, transactions/subscriptions
> migrations) lives in free core, so the provider does too. No new MCP tools/abilities/endpoints.

- [x] 9.1 Create `app/Modules/MCP/Support/PaymentDataProvider.php` (one listener class, no ability registration)
- [x] 9.2 Register it from `MCPInit::init()` (runs only when MCP is enabled)
- [x] 9.3 `fluentform/mcp_submission_data` listener: resolve `form_id` from the submission, return unchanged unless `Acl::hasPermission('fluentform_view_payments', $formId)`, else inject a compact `payment` block; never overwrite an addon-populated key
- [x] 9.4 Build the block via core `OrderData::getTransactions()` / `getSubscriptionsAndPaymentTotal()`, formatting amounts with `PaymentHelper::formatMoney($cents, $currency)`; surface status, formatted total, currency, method, transaction count, subscription status — never raw cents or serialized vendor blobs; `count()` guards (Collections), payment-column pre-check before querying
- [x] 9.5 `fluentform/mcp_submission_rows` listener builds per-row summaries from the seam-passed Submission models (zero queries)
- [x] 9.6 Fail-closed: Throwable or missing capability returns the payload unchanged (never throws into the MCP envelope, never leaks payment data)
- [x] 9.7 Test pins for every prior-issue class; live verification with Pro removed from the runtime (block shown with payments perm, absent on non-payment entries)
- [x] 9.8 `get-payment-summary` advanced read tool: server-side SUM via `ReportHelper::getPaymentsByType` (the admin Reports engine), gated on `fluentform_view_payments`, form-scope-aware site-wide mode, `feature_disabled` error when the payment module is off; definition + guard pins in the suite, live-verified against form 144 ($310 paid, matching the row-level blocks)
