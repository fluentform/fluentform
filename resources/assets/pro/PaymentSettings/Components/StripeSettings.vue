        ...
        <el-skeleton :loading="fetching_connect" animated :rows="10">
            <el-form v-if="settings" label-position="top" rel="offline_settings" :model="settings">
                ...
                <template v-if="settings.is_active == 'yes'">
                    ...
                    <template v-else>
                        <div class="ff_payment_sub_section">
                            ...
                            <p>{{ $t('You can find the API keys to') }} 
                ...
                <div v-if="settings.is_active == 'yes'" class="ff_payment_sub_section">
                    ...
                    <p>
                        {{ $t('In order for Stripe to function completely for subscription/recurring payments, you must configure your Stripe webhooks. Visit your ') }}
                        <a href="https://dashboard.stripe.com/account/webhooks" target="_blank" rel="noopener">
                            {{ $t('account dashboard') }}
                        </a> {{ $t('to configure them. Please add a webhook endpoint for the URL below.') }}
                    </p>
                    <p><b>{{ $t('Webhook URL') }}: </b><code>{{ global_settings.stripe_webhook_url }}</code></p>
                    <p> <a target="_blank" rel="noopener" href="https://wpmanageninja.com/docs/fluent-form/payment-settings/how-to-setup-stripe-ipn-with-wp-fluent-forms/">{{ $t('Please read the documentation') }}</a> {{ $t('to learn how to setup')}} <b>{{ $t('Stripe IPN') }} </b>.</p>
