...
class Payment_handler {
    ...
    init() {
        ...
        this.$form.on('fluentform_next_action_payment', (event, data) => {
    ...
    initSquareElement() {
        if (this.$form.hasClass("ff_has_square_inline")) {
            const inlineElementId = this.$form
                .find(".ff_square_card_element")
                .attr("id");
            ...
            const applicationId = this.formPaymentConfig.square.inline_config.settings.application_id;
            const locationId = this.formPaymentConfig.square.inline_config.settings.location_id;
            ...
            try {
                ...
                const statusContainer = document.getElementById(inlineElementId);
            ...
            try {
                ...
                card.then((cardInstance) => {
                    cardInstance.attach("#" + inlineElementId);

                    this.squareCard = cardInstance;
                    ...
                    this.registerSquarePaymentToken(inlineElementId);
                });
            } catch (error) {
    ...
    registerSquarePaymentToken(inlineElementId) {
        var that = this;
        this.formInstance.addGlobalValidator(
            "squareInlinePayment",
            function ($theForm, formData) {
                if (that.paymentMethod === "square" && !that.hasPaymentItems) {
                    if (
                        !jQuery("#" + inlineElementId).closest(".ff_excluded")
                            .length
                    ) {
                        ...
                        that.squareCard
                            ...
                            .then(function (result) {
                                if (result.status !== "OK") {
                                    ...
                                    // Verify buyer
                                    that.square.verifyBuyer(token, verificationDetails)
