<template>
    <div>
        <label
            ...
            class="label-block"
            :class="item.settings.required ? 'is-required' : ''"
            v-html="item.settings.label"
        ></label>

        <el-row :gutter="20">
            <template v-for="(field, i) in getOrderFields">
                <el-col
                    :key="i"
                    :md="is_conversion_form ? 24 : 12"
                    v-if="getField(field).settings.visible"
                    class="address-field-wrapper"
                >
