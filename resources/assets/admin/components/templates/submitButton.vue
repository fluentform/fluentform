<template>
    <div
        ...
        :class="[
            'text-' + submitButton.settings.align,
            { selected: !!editItem.uniqElKey && editItem.uniqElKey === submitButton.uniqElKey },
        ]"
    >
        ...
        <template v-if="submitButton.settings.button_ui">
            ...
            <img
                v-else
                :src="submitButton.settings.button_ui.img_url"
                :alt="$t('Submit Button')"
        ...
        <!-- Button before 1.2.6 -->
        <button class="ff-btn" :class="btnSize" v-if="submitButton.settings.btn_text" :style="btnStyles">
