<template>
    <div class="ff_choose_template_wrap" :class="{'ff_backdrop': visibility}">
        ...
        >
            <template #header>
                <h3 class="title">
                    {{ $t(`Create Using ${ai_model === "chat_gpt" ? $t("ChatGPT") : $t("AI")}`) }}
                </h3>
                <p>
                    {{ $t(`Use AI to create the initial structure. ${ai_model === "chat_gpt" ? "The response uses ChatGPT, so please" : "Please"} note that there might be some inaccuracy in the output.`)
                    }}
                </p>
            </template>
            ...
            <div class="mt-6">
                <el-form class="mt-4" :model="{}" label-position="top">
                    ...
                    <el-form-item class="ff-form-item" :label="$t('Create a form for')">
                        <el-input ref="queryInput" :placeholder="$t('Customer Review for product')" type="textarea"
                                  v-model="query">
                        </el-input>
                    </el-form-item>
                    ...
                    <el-form-item class="ff-form-item" :label="$t('Including these questions')">
                        <el-input :placeholder="$t('User satisfaction level, most liked and disliked features')"
                                  type="textarea" v-model="additional_query">
                        </el-input>
                    </el-form-item>
        ...
        <div class="ff-ai-creating-form-loading-wrap" :class="{'ff_backdrop': loading}">
            ...
            >
                <div class="ff-ai-creating-form-loading-container">
                    ...
                    <p>{{ $t("Creating your form") }} <span
                        class="typing-indicator"><span>.</span><span>.</span><span>.</span></span></p>
                </div>
            </el-dialog>
        </div>
    </div>
