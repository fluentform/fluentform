<template>
    <div class="quiz-field-container">
        ...
        <el-collapse-transition>
            <div v-if="input.enabled === true && input.has_advance_scoring === 'no' && !is_personality_quiz"
                 class="quiz-field">
                <div class="quiz-field-setting">
                    <div class="lead-title mb-2">{{ $t("Score") }}</div>
                    <el-input-number v-model="input.points" :min="1" :max="100" controls-position="right"
                                     size="small" />
                </div>
                <div class="quiz-field-setting" v-if="ifNeedsCondition(input.element) && hasOptions(input)">
                    ...
                    <el-select v-model="input.condition" @change="resetValue(input)" style="width: 100%" size="small"
                               :placeholder="$t('Select Condition')">
                        <el-option v-for="(label, key) in matchingCondition" :key="key" :label="$t(label)"
                                   :value="key" />
                    </el-select>
                </div>
                <div class="quiz-field-setting" v-if="ifNeedsCondition(input.element)">
                    ...
                    <el-input v-else v-model="input.correct_answer" size="small"
                              :placeholder="$t('Correct Answer')"></el-input>
                </div>
                <div class="quiz-field-setting" v-else-if="isRadioInput(input.element)">
                    <div class="lead-title mb-2"> {{ $t("Correct Answer") }}</div>
                    <el-select
                        multiple
                        size="small" style="width: 100%" v-model="input.correct_answer"
        ...
        <el-collapse-transition v-if="input.has_advance_scoring === 'yes'">
            <div>
                <div v-for="(item, key) in original_input.options" :key="key" class="quiz-field">
                    <div class="quiz-field-setting">
                        <el-input-number v-model="input.advance_points[key]" :min="0" :max="100"
