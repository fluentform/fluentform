        ...
        <el-skeleton :loading="!formSettings" animated :rows="14" :class="!formSettings ? 'ff_card' : ''">
            <template v-if="formSettings">
                ...
                <card id="confirmation-settings">
                    <card-head>
                        <card-head-group class="justify-between">
                            ...
                            <btn-group>
                                <btn-group-item>
                                    <video-doc btn_size="large" :btn_text="$t('Learn More')"
                                               route_id="formConfirmation"/>
                                </btn-group-item>
                                <btn-group-item>
                                    <el-button
                                        :loading="loading"
                ...
                <card v-if="double_optin" id="double-optin-confirmation">
                    ...
                    <card-body>
                        <el-checkbox true-value="yes" false-value="no" v-model="double_optin.status">
                            {{ $t('Enable Double Optin Confirmation before Form Data Processing') }}
                        </el-checkbox>

                        <el-form class="mt-4" v-if="double_optin.status === 'yes'" :data="double_optin"
                                 label-position="top">
                            <el-form-item class="ff-form-item">
                                ...
                                <el-select class="ff_input_width" v-model="double_optin.email_field"
                                           size="large"
                            ...
                            <template v-if="double_optin.email_field">
                                <el-form-item class="ff-form-item">
                                    ...
                                    <p class="mt-1 fs-14">
                                        {{ $t('This message will be shown after the intial form submission') }}</p>
                                </el-form-item>
                                ...
                                <template v-if="double_optin.email_body_type === 'custom'">
                                    ...
                                    <el-form-item class="ff-form-item">
                                        ...
                                        <wp-editor v-else :height="150" :editor-shortcodes="editorShortcodes"
                                                   v-model="double_optin.email_body"/>
                                        <el-checkbox class="mt-3" true-value="yes" false-value="no"
                                                     v-model="double_optin.asPlainText">
                                            {{ $t('Send Email as RAW HTML Format') }}
                                        </el-checkbox>

                                        <p class="mt-2 fs-14">{{
                                                $t('Use #confirmation_url# smartcode for double optin confirmation URL')
                                            }}</p>
                                    </el-form-item>
                                </template>
                                ...
                                <div class="form_item">
                                    <el-checkbox true-value="yes" false-value="no"
                                                 v-model="double_optin.skip_if_logged_in">
                                ...
                                <div v-if="hasFluentCRM" class="form_item">
                                    <el-checkbox true-value="yes" false-value="no"
                                                 v-model="double_optin.skip_if_fc_subscribed">
                ...
                <card v-if="admin_approval" id="admin_approval">
                    <card-head>
                        ...
                        <p class="text">
                            {{
                                $t('Enable admin approval email notifications to inform the admin of pending submissions from')
                            }} <a href="?page=fluent_forms_settings#admin_approval">{{ $t('Global Settings') }}</a>. {{
                                $t('After approve form data & email notification will be processed.You can configure an email for users declined submissions.')
                            }}</p>
                    </card-head>
                    <card-body>
                        ...
                        <el-form label-position="top">
                            <el-row :gutter="24">
                                <el-col>
                                    <el-checkbox true-value="yes" false-value="no" v-model="admin_approval.status">
                                ...
                                <el-col v-if="admin_approval.status === 'yes'">
                                    <el-form-item class="ff-form-item">
                                        <template #label>
                                            ...
                                            <el-tooltip class="item" placement="bottom-start"
                                                ...
                                                <template #content>
                                                    <p>
                                                        {{ $t('Enter the text you would like the user to see just after the form submission.') }}</p>
                                                </template>
                                                <i class="ff-icon ff-icon-info-filled text-primary"/>
                                            </el-tooltip>
                                        </template>
                                    ...
                                    <el-form-item class="ff-form-item">
                                        ...
                                        <el-select class="ff_input_width" clearable v-model="admin_approval.email_field"
                                                   size="large"
                                                   :placeholder="$t('Select an email field')">
                                            <el-option
                                                v-for="(item, index) in emailFields"
                                    ...
                                    <template v-if="admin_approval.email_field">
                                        <el-form-item class="ff-form-item">
                                            ...
                                            <el-input :placeholder="$t('Email Subject')"
                                                      v-model="admin_approval.email_subject"/>
                                        </el-form-item>
                                        <el-form-item class="ff-form-item">
                                            <template #label>
                                            ...
                                            <wp-editor v-else :height="150" :editor-shortcodes="editorShortcodes"
                                                       v-model="admin_approval.email_body"/>
                                            <el-checkbox class="mt-3" true-value="yes" false-value="no"
                                                         v-model="admin_approval.asPlainText">
                                                {{ $t('Send Email as RAW HTML Format') }}
                                            </el-checkbox>
                                    ...
                                    <div class="form_item">
                                        <el-checkbox true-value="yes" false-value="no"
                                                     v-model="admin_approval.skip_if_logged_in">
                ...
                <card id="form-layout">
                    <card-head>
                        <card-head-group class="justify-between">
                            ...
                            <video-doc btn_size="large" :btn_text="$t('Learn More')" route_id="formErrorMessage"/>
                        </card-head-group>
                    </card-head>
                    <card-body>
                            ...
                            <el-row :gutter="24">
                                <el-col :sm="24" :md="6">
                                    ...
                                    <el-form-item class="ff-form-item">
                                        ...
                                        <el-select class="w-100 ff-input-s1"
                                                   size="large"
                ...
                <card id="scheduling-and-restrictions">
                    <card-head>
                        <card-head-group class="justify-between">
                            ...
                            <video-doc btn_size="large" :btn_text="$t('Learn More')" route_id="formScheduling"/>
                        </card-head-group>
                    </card-head>
                    <card-body>
                        ...
                        <div class="ff_settings_section">
                            <div class="ff_settings_body">
                                <form_restriction :data="formSettings.restrictions"
                                                  :has-pro="hasPro"></form_restriction>
                            </div>
                        </div>
                    </card-body>
                </card>
                ...
                <card id="advanced-form-validation">
                    <card-head>
                        ...
                        <p class="text">
                            ...
                            <a target="_blank" rel="noopener"
                               href="https://wpmanageninja.com/docs/fluent-form/advanced-features-functionalities-in-wp-fluent-form/advanced-form-validation-in-wp-fluent-forms-wordpress-plugin/">
                                {{ $t('Learn More') }}
                            </a>
                        </p>
                    </card-head>
                    ...
                    <card-body>
                        <advanced-validation v-if="hasPro" :hasPro="hasPro" :inputs="inputs"
                                             :settings="advancedValidationSettings"></advanced-validation>

                        <notice class="ff_alert_between" type="danger-soft" v-if="!hasPro">
                            <div>
                                <h6 class="title">
                                    {{ $t('Advanced Form Validation is available in the pro version') }}</h6>
                ...
                <card id="compliance-settings">
                    ...
                    <card-body>
                        <el-checkbox v-if="hasPro" true-value="yes" false-value="no"
                        ...
                        <div v-if="formSettings.delete_entry_on_submission !== 'yes'"
                             class="ff_auto_delete_section mt-3">
                            <el-checkbox
                                v-if="hasPro"
                                true-value="yes"
                                false-value="no"
                ...
                <card v-if="affiliate_wp">
                    ...
                    <card-body class="el-form--label-top">
                        <div class="el-form-item ff-form-item">
                            ...
                            <el-checkbox true-value="yes" false-value="no" v-model="affiliate_wp.status">
                                {{ $t('Enable') }}
                            </el-checkbox>
                        </div>
                        <div class="el-form-item ff-form-item">
                            ...
                            <el-select class="ff_input_width" v-model="affiliate_wp.selected_type"
                                       size="large"
                    ...
                    <card-body class="el-form--label-top">
                        <div v-if="hasPro">
                            <div class="el-form-item ff-form-item ff-form-item-flex">
                                ...
                                <div class="el-form-item__content">
                                    <el-switch class="el-switch-lg" v-model="formSettings.conv_form_per_step_save"
                                               v-if="hasPro"/>
                                </div>
                            </div>
                            <transition name="slide-down">
                                <div v-if="formSettings.conv_form_per_step_save"
                                     class="el-form-item ff-form-item ff-form-item-flex">
                                    <label class="el-form-item__label" style="width: 390px;">
                                        {{ $t('Resume from last step') }}
                                    </label>
                                    <div class="el-form-item__content">
                                        <el-switch class="el-switch-lg"
                                                   v-model="formSettings.conv_form_resume_from_last_step"
                                                   v-if="hasPro"/>
                                    </div>
                                </div>
                            </transition>
                        ...
                        <notice class="ff_alert_between" type="danger-soft" v-else>
                            <div>
                                <h6 class="title">
                                    {{ $t('Conversation Form Per Step Save is available in the pro version') }}</h6>
                                <p class="text">{{ $t('Upgrade to get access to all the advanced features.') }}</p>
                            </div>
                            <a target="_blank"
                               href="https://fluentforms.com/pricing/?utm_source=plugin&amp;utm_medium=wp_install&amp;utm_campaign=ff_upgrade&amp;theme_style=twentytwentythree"
...
import form_restriction from './FormSettings/Restrictions.vue';
import SurveyResult from './FormSettings/SurveyResult.vue';
import errorView from '@/common/errorView.vue';
import AddConfirmation from './Includes/AddConfirmation.vue'
import AdvancedValidation from "./Includes/AdvancedValidation.vue";
...
export default {
    ...
    created() {
        this.eventBus.emit('change-title', 'Form Settings');
    }
};
</script>
