        ...
        <vddl-draggable class="panel__body--item js-editor-item"
            ...
            <div ref="container" v-if="item.element == 'container'" class="item-container">
                <template v-for="(containerRow, index) in item.columns">
                    <el-tooltip class="item" effect="dark" :content="`width: ${containerRow.width ? containerRow.width : Math.floor(100 / item.columns.length)}%, left: ${containerRow.left ? containerRow.left : 0}px`" placement="right">
                        <vue-resizable
