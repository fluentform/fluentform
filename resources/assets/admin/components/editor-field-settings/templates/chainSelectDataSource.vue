    ...
    <div class="chained-select-settings">
        <el-form-item>
            ...
            <el-radio-group v-model="dataSourceType">
                <el-radio value="file">{{ $t('File Upload') }}</el-radio>
                <el-radio value="url">{{ $t('Remote URL') }}</el-radio>
            </el-radio-group>
            ...
            <div v-else style="margin-top: 10px">
                <el-input
                    class="chain-select-upload-button"
                    :placeholder="$t('Please input a remote csv url...')"
                ...
                <el-button type="text" class="pull-right btn-danger" :loading="removing" @click="deleteDataSource"
                    >{{ $t('Clear Data Source') }}
                </el-button>
            </div>
        </el-form-item>
        <p>
            <small
                ><a :href="sample_csv_url" target="_blank">{{ $t('Download Sample CSV') }}</a></small
            >
        </p>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        deleteDataSource() {
            ...
            const data = Object.assign({}, this.dataSourceInfo, { action: 'fluentform_chained_select_remove_ds' });
