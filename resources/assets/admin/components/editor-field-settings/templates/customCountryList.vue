<template>
    <div>
        <el-form-item :label="$t('Default Country')">
            <el-select
                v-if="!listItem.disable_labels"
                filterable
                clearable
                size="default"
        ...
        <el-form-item :label="$t('Country List')">
            <el-radio-group size="small" v-model="settings[listItem.key].active_list">
                <el-radio-button value="all">{{ $t('Show all') }}</el-radio-button>
                <el-radio-button value="hidden_list">{{ $t('Hide these') }}</el-radio-button>
                <el-radio-button value="visible_list">{{ $t('Only show these') }}</el-radio-button>
                <el-radio-button value="priority_based">{{ $t('Priority Based') }}</el-radio-button>
            </el-radio-group>
            <el-select
                clearable
                filterable
                ...
                class="el-fluid"
            >
                <el-option v-for="(name, key) in countries" :key="key" :label="name" :value="key"></el-option>
            </el-select>
        </el-form-item>
