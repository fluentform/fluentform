<template>
    <div class="ff-edit-history-wrap option-fields-section" style="min-height: 100px;">
        ...
        <ul class="timeline" v-if="Object.keys(historyData).length">
            <li
                class="timeline_item"
                v-for="(editHistory, i) in historyData"
                :key="i"
                @mouseleave="renderHistory(i, 'leave')"
            ...
            <span class="timeline_date" @click="showDetail(i)">
                ...
                <i :class="showDetails[i] ? 'el-icon-caret-top' : 'el-icon-caret-bottom'"></i>
                    ...
                    <transition name="slide-fade">
                        <div
                            class="timeline_details"
                            :class="{ 'is-visible': showDetails[i] }"
                        >
        ...
        <div v-else class="option-fields-section--content">
            <strong>{{
                $t("To see your recent changes in the history:")
            }}</strong>
            <ul class="list-group">
                <li>{{ $t("Save your current form changes.") }}</li>
                <li>{{ $t("The saved version will then appear in the form history.") }}
                </li>
                <li>{{ $t('Preview or restore from this latest history entry.') }}</li>
            </ul>
        </div>
    </div>
</template>
