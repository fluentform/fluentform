<template>
    <div class="add_on_modules">
        <div class="modules_header mb-5">
            ...
            <p class="text">
                {{
                    $t('Here is the list of all Fluent Forms modules. You can enable or disable the modules based on your need.')
                }}
            </p>
        </div>
            ...
            <el-row class="mb-3" :gutter="24">
                <el-col :span="18">
                    <div class="ff_module_selectors">
                        <el-radio-group class="ff_radio_group" v-model="module_type" size="large">
                            <el-radio-button value="all">{{ $t('All') }}</el-radio-button>
                            <el-radio-button value="crm">{{ $t('CRM & SASS Integrations') }}</el-radio-button>
                            <el-radio-button value="wp_core">{{ $t('WP Core Modules') }}</el-radio-button>
                        </el-radio-group>
                    </div>
                </el-col>
                <el-col :span="6">
                    <div class="ff_modules_search">
                        <el-input :placeholder="$t('Search Modules')" v-model="search"
                                  size="large">
                            <template #prefix>
                        ...
                        <div class="ff_card_footer">
                            <div class="ff_card_footer_group">
                                <template v-if="addon.purchase_url">
                                    <a class="el-button el-button--primary el-button--soft el-button--small"
                                       rel="noopener" :href="addon.purchase_url" target="_blank">{{
                                            $t('Upgrade To Pro')
                                        }}</a>
                                </template>
                                ...
                                <a style="font-size: 22px;" class="text-secondary"
                                   v-if="addon.config_url && addon.enabled === 'yes'" :href="addon.config_url">
...
export default {
    ...
    data() {
        return {
            search: '',
            addOns: JSON.parse(JSON.stringify(window.fluent_addon_modules.addons)),
        ...
        saveStatus(addonKey) {
            ...
            FluentFormsGlobal.$rest.post(url, {
                ...
                .then((response) => {
                    ...
                    this.addOns[addonKey].enabled = this.addOns[addonKey].enabled === 'yes' ? 'yes' : 'no';
                })
                .catch(error => {
                    this.$fail(error.message);
                    this.addOns[addonKey].enabled = this.addOns[addonKey].enabled === 'yes' ? 'no' : 'yes';
                });
        }
    },
    watch: {
        addOns: {
            handler(newAddOns, oldAddOns) {
                each(newAddOns, (newAddon, addonKey) => {
                    if (oldAddOns[addonKey] && newAddon.enabled !== oldAddOns[addonKey].enabled) {
                        this.saveStatus(addonKey);
                    }
                });
            },
            deep: true
