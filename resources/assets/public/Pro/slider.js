...
class FluentFormSlider {
    ...
    stepProgressBarHandle(stepData) {
         ...
         * We consider a step "empty" only if it contains field groups, but all of them are excluded by conditions
         * This avoids skipping steps that intentionally contain only custom HTML or summary content
        ...
        isStepAllFieldsHidden($step) {
            ...
            if ($groups.length === 0) {
                // No field groups means it's not "empty due to conditions"; do not skip
                return false;
            }
    ...
    updateSlider(goBackToStep, animDuration, isScrollTop = true, actionType = 'next') {
        ...
        return new Promise((resolve) => {
            ...
            // Use display:none/block instead of slider and only show the resolved active step
