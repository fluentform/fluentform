...
(function ($) {
    $(document.body).on('fluentform_init', function (e, $theForm, form) {
        ...
        let hash = -1;
        let activeStep = 'no';
        let hasSaveProgress = $(formSelector).hasClass(
            'ff-form-has-save-progress',
        );
        if (!hasSaveProgress) {
            return;
        }
                ...
                $saveBttn.on('click', function (e) {
                    ...
                    $inputs
                        .filter((i, el) => {
                            let $el = $(el);
                            return (
                                $el.parents().hasClass('ff_repeater_table') &&
                                $el.attr('type') == 'select' &&
                                !$el.val()
                            );
                        })
                    ...
                    const saveProgressMessage =
                        formData.form_id + '_save_progress_msg';
                        ...
                        .then(data => {
                            if (data) {
                                ...
                                const copyIcon =
                                    ...
                                    'Copy';
                                let inputDiv = `<div class="ff-el-input--content">
                                ...
                                //Show Email Input
                                const emailPlaceholderStr =
                                    window.form_state_save_vars
                                        .email_placeholder_str ||
                                    'Your Email Here';
                                const emailIcon =
                                    window.form_state_save_vars.email_button ||
                                    'Email';
                                if (
                                    $(this).hasClass('ff_resume_email_enabled')
                                ) {
                                    let emailDiv = `<div class="ff-el-input--content">
                                    <div class="ff_input-group">
        ...
        $(formSelector).on('click', '.ff_btn_copy_link', function (e) {
            ...
            let copiedText = $(this)
                ...
                .find('.ff-el-form-control')
                .val();
            navigator.clipboard.writeText(copiedText);
            const copySuccess =
                window.form_state_save_vars.copy_success_button || 'Copied';
        ...
        $(formSelector).on('click', '.ff_btn_is_email', function (e) {
            ...
            const emailResponse =
                formData.form_id + '_save_progress_email_response';
        ...
        $theForm.append(
            `<input type="hidden" value="${hashKey}" class="__fluent_state_hash" name="__fluent_state_hash"/>`,
        );

        jQuery
            .getJSON(fluentFormVars.ajaxUrl, {
                form_id: $theForm.data('form_id'),
